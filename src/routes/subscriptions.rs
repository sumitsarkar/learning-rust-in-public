...
pub enum SubscribeError {
    ...
    // Transparently delegates both `Display`'s and `source`'s implementation to the type wrapped by `UnexpectedError`.
    #[error("{1}")]
...
pub async fn subscribe(
    ...
    let mut transaction = pool
        .begin()
        .await
        .map_err(|e| SubscribeError::UnexpectedError(Box::new(e), "Failed to acquire a Sqlite connection from the pool".into()))?;
